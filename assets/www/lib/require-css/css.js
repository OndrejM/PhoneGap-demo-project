...
define(['require', './normalize'], function(require, normalize) {
  ...
  cssAPI.load = function(cssId, req, load, config, parse) {
    ...
    var alwaysInjectUsingLink = config.requireCss && config.requireCss.alwaysInjectUsingLink;
    if (alwaysInjectUsingLink || fileUrl.substr(0, 7) == 'http://' || fileUrl.substr(0, 8) == 'https://') {
      if (parse)
